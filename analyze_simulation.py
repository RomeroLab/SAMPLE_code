...
def get_simulation_data(run_date: str='20221130',
    ...
    for strat in strategies:
        strat_data = pickle.load(open(f'simulations/{run_date}_{dataset}_run/{strat}.p','rb'))
...
def learning_curve(run_T50s,
    ...
    T50s = np.array([float(s[1]) for s in 
                     load_data.read_data(f'simulations/{dataset}_thermostability.data')[1]])
    
    temp_thresh = (max(T50s)-min(T50s))*0.9 + min(T50s) # 90% of the max
    save_fn=f'simulations/Images/{run_date}_{dataset}_lc'
...
def iteration_boxplot(run_T50s,
    ...
    temp_thresh = (max(T50s)-min(T50s))*0.9 + min(T50s) # 90% of the max
    save_fn=f'simulations/Images/{run_date}_{dataset}_bw'
    ...
    plt.ylabel('Trials needed to reach 61.88C')
...
def iteration_histogram(run_T50s,
    ...
    temp_thresh = (max(T50s)-min(T50s))*0.9 + min(T50s) # 90% of the max
    save_fn=f'simulations/Images/{run_date}_{dataset}_hist'
    ...
    plt.ylabel('Density')
    plt.xlabel('Trials needed to reach 61.88C')
    plt.title('Iterations needed to reach 90th percentile')
